...
def inference(args, chat):
    ...
    qs = load_file('/data/bhavya/task_verification/CVVREvaluation/cvvr_evaluation_suite/Video-LMMs-Inference/TimeChat/error_prompts/preparation_error.json')
    ...
    for v in os.listdir(video_dir):
        ...
        for steps in questions:
            ...
            print(output)
            pred[question_ind[inp1]] = op_val(pred, correctans, q_type)
            if 'followup' in steps.keys():
                for q in steps['followup']:
                    inp2 = q
                    pred2 = ask_question(args, chat, chat_state, img_list, inp2)
                    print(pred2)
                    pred[question_ind[inp2]] = op_val(pred2, correctans, q_type)
